...
# for(i in 1:10){
#   print(i)
#   sim <- sim_friedman(n = 100, 
#                       pars =  c(rnorm(1, sd = 10), 20, rnorm(1, sd = 10), 5))
#   X <- sim$X
#   y <- sim$y
#   df_sim <- data.frame(y, X, group = i)
#   df <- bind_rows(df, df_sim)
# }
# df |> 
#   ggplot(aes(X2, y)) +
#   geom_point() +
...
sim_tree = function(n = 150, scale_err = 1, m = 5) {
  
  mus  <-  rnorm(n = m, sd = 5)
  pars <-  matrix(rnorm(n = m*4, sd = 3), ncol = 4)
  ms   <-  1:m
  df2  <-  data.frame()
  ...
  for(mm in 1:m){
    ...
    Y = (params[1]*I(X1<1)*I(X2<0.2)+
           params[2]*I(X1<1)*I(X2>=0.2)+
           params[3]*I(X1>=1)
         + params[4]*I(X3>=2))
    Y = Y + rnorm(n, mean = mus[mm], sd = 1)
    mydat = data.frame(y = Y,X1,X2, X3, group = mm)
    df2 <- bind_rows(mydat, df2)
  }
...
write_rds(lme_step, "model_files/simulated_data.rds")  
...
ggsave(file = "paper/boxplot_simulated.png",
       width = 6, height = 3)
# -------------------------------------------------------------
mean_sigma_phi <- hebart_step$hebart_results |> 
  map("sigma_phi") |> 
  map_dbl(mean)

mean_mse <- hebart_step$hebart_results |> 
  map("mse") |> 
  map_dbl(mean)

refs <- lme_step$lme_results |> 
  map_dbl(~{
    b <- parameters::model_parameters(.x, effects = "random")
    b$Coefficient[1]
  })
refs

mse_lme <- lme_step$lme_results |> 
  map_dbl(~{
    res <- residuals(.x)
    r.rmse <- mean(res^2)
    r.rmse
...
(all$mean_mse/all$mean_sigma_phi) |> mean()
(all$mse_lme/all$ref_lme) |> mean()
all |> 
  ggplot(aes(x = mean_sigma_phi, y = mean_mse)) +
  geom_line(linetype = 'dotted') +
  geom_point(colour = "#F96209") +
  theme_fira()
diagnostics(lme_step$hebart_results[[5]])
# -------------------------------------------------------------
# Compare LME to sampled values
my_lme <- lme_step$lme_results[[5]]
...
df_gammas <- tibble(sampled = rgamma(1000, shape_sigma_phi, 1/scale_sigma_phi))
df_gammas <- df_gammas |> 
  filter(sampled < 1.2)
mean(df_gammas$sampled)

df <- tibble(sampled = lme_step$hebart_results[[5]]$sigma_phi)

df |> 
  ggplot(aes(x = sampled)) +
  geom_density(fill = "#F96209", alpha = 0.7) +
  geom_density(
    data = df_gammas, aes(x = sampled), 
    alpha = 0.7) +
  labs(y = "Sampled values", 
       x = 'Density'
  ) + 
  scale_y_continuous(breaks = scales::pretty_breaks(n = 5)) +
  theme_fira()
